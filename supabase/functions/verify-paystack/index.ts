...
type BasicResponse = {
  ...
  json: (payload: Record<string, unknown>) => BasicResponse
}
...
export default async function handler(req: BasicRequest, res: BasicResponse) {
  try {
    ...
    const verifyJson = (await verifyRes.json()) as PaystackVerification
    ...
    if (SENDGRID_API_KEY) {
      ...
      const { data: userRows } = await supabaseAdmin
        ...
        .eq('id', student_id)
        .limit(1)
      const student = (userRows ?? [])[0] as UserRow | undefined
      if (student?.email) {
        const mail = {
          personalizations: [
            {
